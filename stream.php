<?php
	...
	if(isset($_REQUEST["jwt"])){
		try{
			if($verifyJWT($_REQUEST["jwt"])){
				...
				throw new Exception("jwt invalido", 1);
			}
		} catch(Exception $e){
	...
	try{
		if($site == "*" || in_array($_SERVER["HTTP_HOST"], $origins)){
			...
			throw new Exception("Host no permitido: " . $_SERVER["HTTP_HOST"], 1);
		}
			...
			if(APIKEY !== '' && !in_array($method, $method_wl)){
		        ...
		        foreach (getallheaders() as $header => $val) {
		        	if($header == "Authorization") {
		        		$val = str_replace(",", "", $val);
		        		$val = explode(" ", $val);
		        		if(strripos($val[0], "Bearer") === false) throw new Exception("Llamada no autorizada: Header invalido. ");
		        ...
		        if($not_authorized) throw new Exception("Llamada no autorizada: Header vacio. ");
		    }
			$API->checkAPPState();
		...
		if($method == "views"){
			...
			if($response["error"] == "permision error: Action not allowed.") {
